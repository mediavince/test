...
//if (($_REQUEST['adminName'] == 'admin') && (md5($_REQUEST['passWord']) == '21232f297a57a5a743894a0e4a801fc3')) {
  if (!isset($_POST['send']) && !isset($_POST['config']) && !isset($_POST['params']) && !isset($_POST[$upldbString])) {
    
    if($_SERVER['REQUEST_METHOD'] == "POST"){
       Die("Unauthorized attempt to access page.");
    }
    
    $content .= '<h1>1. Select a new SQL file if you wish to add new tables directly to the template<br />2. Fill in the information<br />3. Click the install button...</h1><form method="POST" action="" enctype="multipart/form-data"><input type="hidden" name="'.$upldbString.'" value="'.$upldbString.'" /><input type="file" name="sqlupload" /><br />';
    // lang manager
    $content .= '<br />Select the languages you wish to have available in the system, make sure to enable the array_lang accordingly...<br />';
    foreach($array_supported_lg as $keylg)
  //  if (@file_exists('../images/tab_inverted_'.$k.'.gif'))
    if (@file_exists($up.'images/'.$keylg.'.gif'))
    $content .= ($keylg==$array_supported_lg[0]?'':' | ').' <strong>'.$keylg.'</strong> <input type="checkbox" name="lang_'.$keylg.'" '.($keylg==$array_supported_lg[0]?$inputchecked:'').' /> <img src="'.$up.'images/'.$keylg.'.gif" title="'.sql_stringit('lang',$keylg).'" alt="'.sql_stringit('lang',$keylg).'" style="height:24px;width:24px;" /> ';//tab_inverted_'.$k.'.gif" />'
    $content .= '<br /> <br />';
 ...
 //   if (isset($_POST['params']))
    ...
    if ($update_rapport == '') {
      ...
      if ($update_rapport == '') {
        ...
        $connection = connect();
        $db_exists = @mysql_query("USE $dbname;");
        ...
        //	die($content);
            ...
            $content .= '<h1 style="text-align:center">NOT connected: the database could not be created !!</h1>'.$do;
        ...
        //
				...
				@mysql_close($connection);
        $connection = connect();
        $tbls_exist = @mysql_query("DESCRIBE _admin;");
        if (!$tbls_exist) {
        ...
        if (isset($_POST[$upldbString]) && isset($_FILES)) {
          ...
          if (count($chosen_lang)>0) {
            ...
            foreach($chosen_lang as $keylg) {
	            $insert_cont = @mysql_query("INSERT INTO `_cont` VALUES (NULL,'Y',NOW(),'admin',NOW(),'admin',1,'$keylg','MVCMS','<p>The website is ready to be used. Go to the admin directory.</p>','mvcms','1','','','center','','')");
...
';
      		...
      		fclose($inF);
          $content .= '<br /><p style="text-align: center"><font color="Green"><b>htaccess: '.$enregistrementString.' '.$effectueString.'</b></font></p>';
          //
