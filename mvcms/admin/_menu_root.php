...
//(!stristr($_SERVER['REQUEST_URI'],$urladmin) || (stristr($_SERVER['REQUEST_URI'],$urladmin) && ($logged_in === false)))

if (!isset($protected_show))
$protected_show = false;

if (stristr($_SERVER['REQUEST_URI'],$urladmin) && ($logged_in === false)) {
//  $getcwd = getcwd()."/".$up;
}
...
if (isset($x) && ($x !== '1')) {
  $redir_tourl = sql_getone($tblcont,"WHERE $where_statut_lang $where_priv contpg='$x' ","conturl").($htaccess4sef===true?"_$lg.php":'');
...
$x = sql_getone($tblcont,"WHERE contstatut='Y' AND contlang='$default_lg' AND $where_priv conturl='".substr(substr($_SERVER['PHP_SELF'],$extractfromphpself),0,-7)."_$default_lg.php' ","contpg") ; // define x if no redirect
else {
  ...
  $x = sql_getone($tblcont,"WHERE contstatut='Y' AND contlang='$lg' AND $where_priv conturl='$last_piece' ","contpg");
}
  ...
  foreach($array_modules as $key) {
    if (isset(${$key."Id"}) && preg_match("/^[0-9]+\$/",${$key."Id"})) {
      ...
      if ($id_type != '') {//&& in_array($id_archive,array('','N'))) {
        $id_type_url = sql_getone($tblcont,"WHERE contstatut='Y' AND conturl LIKE '%".space2underscore(sql_stringit($key.'type',$id_type))."_$lg."."%' ","contpg");
...
/*
        //  if (!strstr(sql_getone($tblcont," WHERE $where_statut_lang AND contpg='$x' ","contentry"),"[".$get_x[1]))
          $notice .= mvtrace(__FILE__,__LINE__)." $x<br />";
*/
if (($htaccess4sef === true) && ($_SERVER['REQUEST_URI'] != $url_mirror)) {
  ...
  if (count($lastpg)>1) {
    ...
    foreach($array_uri as $key=>$value) {
        ...
        if (in_array($value,sql_array($tblhtaccess,"WHERE htaccessstatut='Y' ","htaccessurl"))) {
          $get_x = sql_get($tblhtaccess,"WHERE htaccessstatut='Y' AND htaccesslang='$lg' AND htaccessurl='$value' ".(isset($potential_type)&&$value!=$potential_type?" AND htaccesstype='$potential_type'":'')." ORDER BY htaccessdate DESC ","htaccessitem,htaccesstype");
          if ($get_x[0] != '.') {
            ${$get_x[1]."Id"} = $get_x[0];
            $this_sql_fields = sql_fields(${"tbl".$get_x[1]},'array');
            $get_info = sql_get(${"tbl".$get_x[1]},"WHERE ".$get_x[1]."rid='".$get_x[0]."' AND ".$get_x[1]."lang='$lg' AND ".$get_x[1]."statut='Y' ",implode(",",$this_sql_fields));//$get_x[1]."type,".$get_x[1]."archive");
            if ($get_info[0]!= '.') {
              ${$get_x[1]."Type"} = $get_info[array_search($get_x[1]."type",$this_sql_fields)];
              $is_archived = $get_info[array_search($get_x[1]."archive",$this_sql_fields)];
              if ((${$get_x[1]."Type"}!='') && in_array($is_archived,array('','N')))
              $new_potential_x = sql_getone($tblcont,"WHERE $where_statut_lang conturl='".space2underscore(sql_stringit($get_x[1]."type",${$get_x[1]."Type"}))."' ","contpg");
              if (isset($new_potential_x) && ($new_potential_x != ''))
              $x = $new_potential_x;
            }
            $potential_x = sql_getone($tblcont," WHERE $where_statut_lang conttype='".$get_x[1]."' ","contpg");
            $potential_array_uri_k = $key;
          }
        }
    }
    if (isset($potential_x) && in_array($x,array('','.'))) {
      $try = sql_get($tblcont,"WHERE $where_statut_lang conturl='".$array_uri[$potential_array_uri_k-1]."' ","contpg,contentry");
      if ($get_x[0] == '.')
      $potential_x = false;
      else {
        if (strstr($try[1],"[".$get_x[1].":") || strstr($try[1],"[".$get_x[1]."]"))
        $x = $try[0];
        else
        $x = $potential_x;
      }
  ...
  if (in_array($x,array('','.')))
  if (!stristr($_SERVER['REQUEST_URI'],$urladmin) && !(($_SERVER['REQUEST_URI'] == $url_mirror) && (substr($_SERVER['PHP_SELF'],$extractfromphpself) == "index.php"))) {
    ...
    $get_x = sql_get($tblcont,"WHERE $where_statut_lang LENGTH(contpg)=$count_lastpg AND conturl='$lastpg_arrayrev' ","contpg,conttype");
    $x = (in_array($get_x[0],array("","."))&&(!in_array($get_x[1],$array_modules)||(in_array($get_x[1],$array_modules)&&in_array(sql_getone($tblcont,"WHERE $where_statut_lang conttype='$get_x[1]' ","contpg"),array("","."))))?"1":$get_x[0]);
    ...
    if (($x == "") || ($x == '.')) {
      ...
      if (isset($tblhtaccess))
      if (in_array($lastpg_arrayrev,sql_array($tblhtaccess,"WHERE htaccessstatut='Y' AND htaccesslang='$lg' ","htaccessurl"))) {// :: check later if relevant here
        $get_x = sql_get($tblhtaccess,"WHERE htaccessstatut='Y' AND htaccesslang='$lg' AND htaccessurl='$lastpg_arrayrev' ","htaccessitem,htaccesslang,htaccesstype,htaccessmetadesc,htaccessmetakeyw");
        if ($get_x[0] != '.') {
          Header("Location: $mainurl".lgx2readable($get_x[1],'',$get_x[2],$get_x[0]).($_SERVER['QUERY_STRING']!=""?'?'.$_SERVER['QUERY_STRING']:''));Die();
        }
      }
      $try = sql_get($tblcont,"WHERE contstatut='Y' AND $where_priv conturl='$lastpg_arrayrev' ","contpg,contlang,conturl");
      if ($try[0]!='.') {
  	  //  $_SESSION['mv_notice'] = "redirected";
        Header("Location: $mainurl".lgx2readable($lg,$try[0]).($_SERVER['QUERY_STRING']!=""?'?'.$_SERVER['QUERY_STRING']:''));Die();
     //   Header("Location: $mainurl".(count($array_lang)>1?$try[1]."/":'').$try[2].($_SERVER['QUERY_STRING']!=""?'?'.$_SERVER['QUERY_STRING']:''));Die();
      }
    }
  } else {
    $x = '1';
  }
  if  (($_SERVER['REQUEST_URI'] == $url_mirror) || ($_SERVER['REQUEST_URI'] == $url_mirror."/") || ($_SERVER['REQUEST_URI'] == $url_mirror.$lg) || ($_SERVER['REQUEST_URI'] == $url_mirror.$lg."/"))
  $x = '1';
}
if (($x == "") || ($x == '.')) {
  $x = '1';
  ...
  if (($_SERVER['REQUEST_URI'] != $url_mirror) && ($_SERVER['QUERY_STRING']=="")) {
    ...
    $notice = '<div style="text-align:center;color:red;font-weight:bold;margin-top:-5px;">'.strtoupper($erreurString).' 404! '.($lg=='it'?'Non Esistente':($lg=='fr'?'Introuvable':'Not Found')).'<br />
<style type="text/css">
  #goog-wm { }
...
$scroller_div = "";
$scroller = sql_get($tblcont," WHERE conttype='scroller' AND contlang='$lg' ","conttitle, contentry");
if ($scroller[0] == '.') {
	$scroller = sql_get($tblcont," WHERE conttype='scroller' AND contlang='$default_lg' ","conttitle, contentry");
}
if ($scroller[0] == '.') {
	...
	$scroller_div = '
    <div id="scroller">
		<marquee behavior="scroll" direction="up" height="50" scrollamount="1" scrolldelay="30" onmouseover=\'this.stop()\' onmouseout=\'this.start()\'>
		<br /> <br />
		<h1>
'.$scrollertitle.'
		</h1>
...
$leftlinks = sql_get($tblcont," WHERE conttype='leftlinks' AND contlang='$lg' ","conttitle, contentry");
if ($leftlinks[0] == '.') {
	$leftlinks = sql_get($tblcont," WHERE conttype='leftlinks' AND contlang='$default_lg' ","conttitle, contentry");
}
if ($leftlinks[0] == '.') {
...
$pgcontent = sql_get($tblcont,"WHERE $where_statut_lang contpg='$x' ","conttitle,contentry,contmetadesc, contmetakeyw,conttype,contlogo");
$title = $pgcontent[0];
$content = $pgcontent[1];
$desc = $pgcontent[2];
$keyw = $pgcontent[3];
...
$default_lg_pg = space2underscore(sql_getone($tblcont,"WHERE contpg='$x' AND contlang='$default_lg' ","conttitle"));
