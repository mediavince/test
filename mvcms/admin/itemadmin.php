...
if (isset($that_is)) {
  ...
  $array_mandatory_fields = array_unique(array_merge($array_mandatory_fields,${"_".$that_is."_array_mandatory_fields"}));
}
...
if ((!in_array($this_is."lang",$array_fields) && !isset($that_is)) || 
    (!in_array($this_is."lang",$array_fields) && isset($that_is) && !in_array($that_is."lang",$that_array_fields)))
$basic_array = array_diff($basic_array,array('lang','rid'));
$params_array = array_merge($basic_array,$params_array);

if (in_array($this_is,$editable_by_membre) && !stristr($_SERVER['REQUEST_URI'],$urladmin) && ($logged_in === true)) {
...
foreach($array_fields as $key) {
  $empty_array_fields[] = ($key=='statut'?'Y':(isset(${$key."Id"})&&in_array($this_is,$editable_by_membre)&&!stristr($_SERVER['REQUEST_URI'],$urladmin)&&($logged_in === true)?"'".${$key."Id"}."'":''));
...
if (isset($that_is)) {
  ...
  if (!isset(${$that_is.'_referrerfield'})) {
    foreach($array_preferred_fields as $k)
      if (!isset(${$that_is.'_referrerfield'}))
        if (in_array($that_is.$k,$that_array_fields)) ${$that_is.'_referrerfield'} = $that_is.$k;
  }
  if (!isset(${$that_is.'_referrerfield'}))
...
if ((isset($rqst) || isset($toggle)) && isset(${$this_is."Id"})) {
 	...
 	if ((isset($toggle) || ($rqst == 'conf') || ($rqst == 'canc')) && preg_match("/^[0-9]+\$/",${$this_is."Id"}) && (${$this_is."Id"} !== '')) {
    if ($this_is == 'admin') {
    //  if (sql_getone($dbtable,"WHERE adminid='".${$this_is."Id"}."' ","adminutil") == $admin_name) {
      if (${$this_is."Id"} == $admin_id) {
        $error = $error_invmiss.' => '.$sortirString.' !';
      } else if (${"nRows".ucfirst($this_is)} == 1) {
        $error = $error_invmiss.' => <b>'.${"nRows".ucfirst($this_is)}.'</b> '.$class_conjugaison->plural($enregistrementString,'M',${"nRows".ucfirst($this_is)}).' '.$surString.' '.${$this_is."String"}.' !';	
      }
    }
    if ($error=='') {
      foreach($array_fields as $k_check) {
        ...
        if (isset(${$field_check."Id"})) {
          ...
          $notice .= '<b>'.$enregistrementString.' '.$pourString.' '.$field_check.' '.$class_conjugaison->plural((mv_toggle(${"tbl".$field_check},$field_check.(in_array($field_check.$toggle,$array_fields)?$toggle:"statut")," ".$field_check.(in_array($field_check."rid",$array_fields)?'r':'')."id='".${$field_check."Id"}."' AND ".$this_is.$field_check."='".${$this_is."Id"}."' ")=='Y'?$confirmeString:$enattenteString),'M',1).'</b>';
    ...
    Header("Location: ".html_entity_decode($local_url.(isset($send)&&isset(${$this_is."Id"})?'&amp;send='.$send.'&amp;'.$this_is.'Id='.${$this_is."Id"}:'')));Die();
 	} else
  {Header("Location: $redirect");Die();}
}
...
foreach($array_int3_have_to_match as $key) {
  ...
  $sql = @mysql_query("SELECT * FROM ".${"tbl".$key[0]}." WHERE ".$key[0]."lg='$lg' ORDER BY ".$key[0].(isset($row[$key[0]."rid"])?'r':'')."id ASC ");
  while($row = @mysql_fetch_array($sql)) {
    if (!isset($current_element_id)) $current_element_id = $row[$key[0].(isset($row[$key[0]."rid"])?'r':'')."id"];
    else {
      if ($row[$key[0].(isset($row[$key[0]."rid"])?'r':'')."id"]!=$current_element_id) $current_element_id = $row[$key[0].(isset($row[$key[0]."rid"])?'r':'')."id"];
    }
    $javascript_for_dependancy_head .= $key[0]."Array[".$row[$key[0].(isset($row[$key[0]."rid"])?'r':'')."id"]."] = \'".html_entity_decode((isset($row[$key[0]."gendre"])?sql_stringit('gendre',$row[$key[0]."gendre"]).' '.$row[$key[0]."prenom"].' '.$row[$key[0]."nom"]:(isset($row[$key[0]."titre"])?$row[$key[0]."titre"]:(isset($row[$key[0]."title"])?$row[$key[0]."title"]:(isset($row[$key[0]."util"])?$row[$key[0]."util"]:(isset($row[$key[0]."type"])?sql_stringit($key[0]."type",$row[$key[0]."type"]):$row[$key[0].(isset($row[$key[0]."rid"])?'r':'')."id"]))))))."\';\n";
    for($i=1;$i<count($key);$i++) {
      $element = $key[$i];
      $javascript_for_dependancy .= "Array_$current_element_id = [];\n";
      $key_sql = @mysql_query("SELECT * FROM ".${"tbl".$element}." ORDER BY ".$element.(in_array($element."rid",sql_fields(${"tbl".$element},'array'))?'r':'')."id ASC ");
      while($row_key = @mysql_fetch_array($key_sql))
      if ($row_key[$element.$key[$i-1]] == $current_element_id)
      $javascript_for_dependancy .= "Array_".$current_element_id."[".$row_key[$element.(isset($row[$element."rid"])?'r':'')."id"]."] = \'".html_entity_decode((isset($row_key[$element."gendre"])?sql_stringit('gendre',$row_key[$element."gendre"]).' '.$row_key[$element."prenom"].' '.$row_key[$element."nom"]:(isset($row_key[$element."titre"])?$row_key[$element."titre"]:(isset($row_key[$element."title"])?$row_key[$element."title"]:(isset($row_key[$element."util"])?$row_key[$element."util"]:(isset($row_key[$element."type"])?sql_stringit($element."type",$row_key[$element."type"]):$row_key[$element.(isset($row[$element."rid"])?'r':'')."id"]))))))."\';\n";
    }
  }
}
...
if (isset($send)) {
  if ($send == ucfirst($envoyerString).' ZIP') {
    include 'galleryzip.php';
  }
  if (($forum_commenting === true) && 
      (($send == $envoyerString.' '.$reponseString) || ($send == $ajouterString.' '.$commentString) ||
  ...
  if ((($send == $envoyerString) || ($send == $sauverString)) && ($_SERVER["REQUEST_METHOD"] == "POST")) {
    include $getcwd.$up.$urladmin.'function-process_post.php';
  }
  $this_is = $old_this;
  $dbtable = $old_dbtable;
  ###################################################################################################
  if (!in_array('1',$admin_priv) || (in_array('1',$admin_priv) && in_array($send,array('edit',$sauverString))))
  if (($send == 'new') || ($send == 'edit') || (($error != '') && (($send == $sauverString) || ($send == $envoyerString)) ) ) {// style="clear:both;"
  //  if (($send == 'new') && ($lg != $default_lg)) $lg = $default_lg;// other langs cannot add new entry
    if (in_array($this_is,$array_galleries) && (($send == 'new') || ($send == 'edit'))) {
      include 'galleryzip.php';
    }
    if (isset($datetime_array[0])) {
      $javascript .= '<script src="'.$mainurl.$urladmin.'jscripts/calendarpopup.js" type="text/javascript" language="javascript"></script><script src="'.$mainurl.$urladmin.'dates.php?lg='.$lg.'" type="text/javascript" language="javascript"></script>';//<script>var cal = new CalendarPopup(\'calpop\');  cal.setWeekStartDay(1); cal.autoHide();</script>
    }
    $content .= '<table align="center" border="0" cellpadding="2" cellpspacing="0"><tr><td align="center">';
    $content .= ($send=='new'?$ajoutString:(isset(${$this_is."Id"})&&preg_match("/^[0-9]+\$/",${$this_is."Id"})&&!in_array('1',$admin_priv)?'<a href="'.$local_url.'&amp;send=delete&amp;'.$this_is.'Id='.${$this_is."Id"}.'" onclick="return confirm(\''.$confirmationeffacementString.'\');"><img src="'.$mainurl.'images/delete.gif" width="10" height="10" alt="'.$effacerString.'" title="'.$effacerString.'" align="right" border="0" /></a> '.(in_array($this_is.'archive',$array_fields)?'<a href="'.$local_url.(isset($send)?'&amp;send='.$send:'').'&amp;'.$this_is.'Id='.${$this_is."Id"}.'&amp;toggle=archive" onclick="return confirm(\''.($editthis[array_search($this_is.'archive',$array_fields)]=='Y'?$nonString.' '.$archiveString.' ?\');"><img src="'.$mainurl.'images/archive_y.png" width="24" height="24" border="0" title="'.$archivedString.'" alt="'.$archivedString:$archiveString.' ?\');"><img src="'.$mainurl.'images/archive_n.png" width="24" height="24" border="0" title="'.$nonString.' '.$archivedString.'" alt="'.$nonString.' '.$archivedString).'" align="right" /></a> ':'').(in_array($this_is.'publish',$array_fields)?'<a href="'.$local_url.(isset($send)?'&amp;send='.$send:'').'&amp;'.$this_is.'Id='.${$this_is."Id"}.'&amp;toggle=publish" onclick="return confirm(\''.($editthis[array_search($this_is.'publish',$array_fields)]=='Y'?$nonString.' '.$publishString.' ?\');"><img src="'.$mainurl.'images/publish_y.png" width="12" height="12" border="0" title="'.$publishedString.'" alt="'.$publishedString:$publishString.' ?\');"><img src="'.$mainurl.'images/publish_n.png" width="12" height="12" border="0" title="'.$nonString.' '.$publishedString.'" alt="'.$nonString.' '.$publishedString).'" align="right" /></a> ':'').$modifierString:'')).' '.${$this_is."String"}.'</td></tr><tr><td align="left">'.(stristr($_SERVER['REQUEST_URI'],$urladmin)?gen_form($lg,$x,$y):'<form enctype="multipart/form-data" name="f'.$x.'" action="'.($full_url===true?$mainurl.lgx2readable($lg,$x):'').'" method="'.$postgetmethod.'"><input type="hidden" name="send" value="'.$sauverString.'" />');
    for($i=0;$i<count($array_fields);$i++) {
      if (substr($array_fields[$i],0,strlen($dbtable)-1) != $this_is) {
        $this_is = $that_is;
        $dbtable = $that_dbtable;
        $content .= '<hr /><h2 style="text-align:center;">'.$paramsString.' '.${$this_is."String"}.'</h2>';
      }
      $key = substr($array_fields[$i],strlen($dbtable)-1);
      if (isset(${$this_is.ucfirst($key)}))
      $editthis[$i] = ${$this_is.ucfirst($key)};
      if (in_array($key,$array_mandatory_fields))// || (isset(${"_".$this_is."_array_mandatory_fields"}) && in_array($key,${"_".$this_is."_array_mandatory_fields"})))
      ...
      if (in_array($key,$basic_array)) {
        if (isset(${$this_is."Id"})) {
          ...
          //  ${$this_is."Id"} = $this_id;
            ...
            $content .= '<input name="'.$this_is.ucfirst($key).'" type="hidden" value="'.$editthis[$i].'" />'.(stristr($_SERVER['REQUEST_URI'],$urladmin)?'> '.${$key."String"}.' <i>'.$editthis[$i].'</i><br />':'');
          }
          if ($key == 'date') {
            if (in_array($key,$array_mandatory_fields)) { // edit
              $javascript .= '<script>var cal'.$i.' = new CalendarPopup(\'calpop'.$i.'\');  cal'.$i.'.setWeekStartDay(1); cal'.$i.'.autoHide();</script>';
              $content .= '<label for="'.$this_is.ucfirst($key).'"><b>> '.${$key."String"}.'</b></label> <input type="text" name="'.$this_is.ucfirst($key).'" value="'.($editthis[$i]!=''?edit_date($editthis[$i]):'DD/MM/YYYY').'" size="12" /> <div class="cal_sml"><a class="toggle" onclick="cal'.$i.'.select(document.forms[\'f'.$x.'\'].'.$this_is.ucfirst($key).',\'anchor'.$i.'\',\'dd/MM/yyyy\'); return false;" name="anchor'.$i.'" id="anchor'.$i.'">'.$calendrierString.'</a></div><div id="calpop'.$i.'" class="calpop" style="visibility:hidden;">&nbsp;</div><br />';
            } else
            $content .= '<input name="'.$this_is.ucfirst($key).'" type="hidden" value="'.$editthis[$i].'" />'.(stristr($_SERVER['REQUEST_URI'],$urladmin)?'> '.${$key."String"}.' <i>'.$editthis[$i].'</i><br />':'');
          }
        } else {
        ...
        if ($key == 'statut') {
          if ($editthis[$i] == '') $editthis[$i] = (stristr($_SERVER['REQUEST_URI'],$urladmin)?'Y':'N');
          if (!in_array('1',$admin_priv))
          $content .= (stristr($_SERVER['REQUEST_URI'],$urladmin)?'<label for="'.$this_is.ucfirst($key).'">> '.${$key."String"}.'</label> <!--<br />--><select name="'.$this_is.ucfirst($key).'">'.gen_selectoption($tblenum,$editthis[$i],'','statut').'</select><br />':'<input name="'.$this_is.ucfirst($key).'" type="hidden" value="'.$editthis[$i].'" />');
        }
        // lg is always set
        if ($key == 'lang')
        $content .= '<input type="hidden" name="'.$this_is.ucfirst($key).'" value="'.$lg.'" />';
      } else if (!stristr($_SERVER['REQUEST_URI'],$urladmin) && in_array($key,$array_hidden)) {
        $content .= '<input name="'.$this_is.ucfirst($key).'" type="hidden" value="'.$editthis[$i].'" />'.(stristr($_SERVER['REQUEST_URI'],$urladmin)?'> '.${$key."String"}.' <i>'.$editthis[$i].'</i><br />':'');
      } else {
      ///////////// routine for dynamic processing
        if ($key == 'util') {
          if (($this_is == 'admin'))
          $content .= '<label for="'.$this_is.ucfirst($key).'">> '.$utilString.' "admin"</label> (a-z A-Z 0-9 _ -)<br /><input name="'.$this_is.ucfirst($key).'" type="text" value="'.$editthis[$i].'" style="width:97%;" autocomplete="off" /><br />';
          else
          $content .= '<label for="'.$this_is.ucfirst($key).'">> '.$utilString.' "membre"</label><br /><input name="'.$this_is.ucfirst($key).'" type="text" value="'.$editthis[$i].'" style="width:97%;" autocomplete="off" /><br />';
        } else if ($key == 'pass')
        $content .= '<label for="'.$this_is.ucfirst($key).'">> '.$motdepasseString.'</label>'.($send=='edit'?' > <b>'.$laisservidepourgarderString.'</b> ':'').($this_is!='admin'?' > <b>'.ucfirst(strtolower($voirString.' '.$confirmationString.' '.$emailString)).'</b>':'').'<input name="'.$this_is.ucfirst($key).'" type="password" value="" style="width:97%;" /><br />';
        else if ($key == 'email')
        $content .= '<label for="'.$this_is.ucfirst($key).'">> '.${$key."String"}.'</label>'.(in_array($this_is,$array_email_conf)?(stristr($_SERVER['REQUEST_URI'],$urladmin)?' > <b>'.ucfirst(strtolower($envoyerString.' '.$confirmationString)).'?</b> <input name="'.$key.'" type="checkbox" /> ':''):'').'<input name="'.$this_is.ucfirst($key).'" type="text" value="'.$editthis[$i].'" style="width:97%;" /><br />';//<input name="'.$key.'" type="hidden" '.$inputchecked.' value="on" />
        else if ($key == 'priv') {
          if (($this_is == 'admin')) {
            if (!in_array('1',$admin_priv))
            $content .= '<label for="privilege">> '.ucfirst($privilegeString).'</label><br /><img src="'.$mainurl.'images/spacer.gif" width="30" height="0" border="0" alt="" /><label for="privilege">'.$pageString.'</label> > <input type="radio" name="privilege" value="1" '.($editthis[$i]==1?'checked="checked"':'').' /> <input type="radio" name="privilege" value="0" '.($editthis[$i]==0?'checked="checked"':'').' /> &nbsp;&nbsp;< <label for="privilege">'.$toutString.'</label><br />';
          } else {
            if ($editthis[$i] == '') $editthis[$i] = '1';
            if ($mod_priv === true)
            $content .= '<br /><div><div style="float:left;"><label for="'.$this_is.ucfirst($key).'"> > <b> '.ucfirst($privilegeString).' : </b></label> &nbsp;</div>'.gen_inputcheck($tblenum,$editthis[$i],'','privilege',true)."</div><br /> <br />";//gen_inputcheck last param true for hiding first item
          }
        } else if (($key == 'title') || ($key == 'titre'))
        $content .= '<label for="'.$this_is.ucfirst($key).'">> '.${$key."String"}.'</label><br /><input name="'.$this_is.ucfirst($key).'" type="text" value="'.$editthis[$i].'" style="width:97%;" /><br />';
        else if (($key == 'website') || ($key == 'url'))
        $content .= '<label for="'.$this_is.ucfirst($key).'">> '.${$key."String"}.'</label><br /><input name="'.$this_is.ucfirst($key).'" type="text" value="'.(!stristr($editthis[$i],"http://")?"http://":'').$editthis[$i].'" style="width:97%;" /><br />';
        else if ($key == 'gendre')
        $content .= '<label for="'.$this_is.ucfirst($key).'">> '.${$key."String"}.'</label> <!--<br />--><select name="'.$this_is.ucfirst($key).'">'.gen_selectoption($tblenum,$editthis[$i],'','gendre').'</select><br />';
        else if ($key == 'comment') {
          if ((isset($send) && ($send != 'new')) && ($admin_viewing === false)) {
            include 'commentmanager.php';
            $content .= $show_comment;
          } else {
            $show_comment = '';
          }
          ...
          if ($array_fields_type[$this_is.$key] == 'text') {
            ...
            if (isset($this_id)) {
              if ($key == "img")
              $editthis[$i] = sql_array($tblcontphoto,"WHERE contphotolang='$lg' AND contphotocontid='$this_id' AND contphotoimg LIKE '%".strtoupper($this_is)."%' ORDER BY contphotosort,contphotoid ASC ","contphotoimg");
              if ($key == "doc")
              $editthis[$i] = sql_array($tblcontdoc,"WHERE contdoclang='$lg' AND contdoccontid='$this_id' AND contdoc LIKE '%".strtoupper($this_is)."%' ","contdoc");
            } else
            $editthis[$i] = array();
          }
          /*
          if ($key == 'img')
          $contentimg = '<div class="image'.($nof>1?'high':'one').'nof">';
          */
          if ($key == 'img')
          $content .= '<div style="float:left;width:100%;margin:0 auto;min-width:550px;">';
        //  $nof = (count($editthis[$i])<$nof||$editthis[$i]==''||$send=='new'?$nof:(in_array($this_is,$array_galleries)?count($editthis[$i])+1:count($editthis[$i])));
          ...
          for($ii=0;$ii<$this_nof;$ii++) {
            if ($key == 'img') {
              $contphoto_img_by_id = sql_getone($tblcontphoto,"WHERE contphotolang='$lg' AND  contphotorid='".$editthis[$i]."' ","contphotoimg");
              if (isset($this_id) && isset($editthis[$i][$ii]))
              $contphoto_desc_by_this_id = sql_getone($tblcontphoto,"WHERE contphotolang='$lg' AND  contphotocontid='$this_id' AND contphotoimg='".$editthis[$i][$ii]."' ","contphotodesc");
              $content .= '<div style="float:left;width:48%;text-align:center;">'.(isset($editthis[$i][$ii])&&$editthis[$i][$ii]!=''?'<br /><img src="'.$mainurl.(is_int($editthis[$i])?$contphoto_img_by_id.'?'.$now_time.'" '.show_img_attr($contphoto_img_by_id):$editthis[$i][$ii].'?'.$now_time.'" '.show_img_attr($editthis[$i][$ii])).' border="0" /><br />':'').'<br />'.'<label for="'.$this_is.ucfirst($key).'[]">> '.${$key."String"}.($ii>0?$ii:'').' '.(isset($editthis[$i][$ii])&&$editthis[$i][$ii]!=''?'<a href="'.$mainurl.(stristr($editthis[$i][$ii],$safedir)?$urlsafe.'?file='.base64_encode($editthis[$i][$ii]):$editthis[$i][$ii]).'" target="_new" class="extlink"><!--'.$editthis[$i][$ii].'--></a> | <a href="'.$local_url.'&amp;send=delete&amp;'.$this_is.'Id='.$editthis[4].'&amp;'.$this_is.ucfirst($key).'='.$editthis[$i][$ii].'" onclick="return confirm(\''.$confirmationeffacementString.'\');"><img src="'.$mainurl.'images/delete.gif" width="10" height="10" alt="'.$effacerString.'" title="'.$effacerString.'" border="0" /></a> ':'').'</label> '.(in_array($this_is,$array_galleries)?'<input class="text" type="text" name="'.$this_is."Desc".'[]" value="'.(isset($that_is)?(in_array($this_is.'nom',$that_array_fields)?$editthat[array_search($this_is.'nom',$that_array_fields)]." ".$editthat[array_search($this_is.'prenom',$that_array_fields)]:$editthat[5]):(isset($contphoto_desc_by_this_id)&&$contphoto_desc_by_this_id!=''?str_replace("-"," ",$contphoto_desc_by_this_id):$editthis[5])).'" /><br />':'').'<input type="file" name="'.$this_is.ucfirst($key).'[]" /><br /></div>';
            } else {
              $content .= '<label for="'.$this_is.ucfirst($key).'[]">> '.${$key."String"}.($ii>0?$ii:'').'</label>';
            //  $content .= ' <!--<input type="hidden" name="'.$this_is."Desc".'[]" value="'.strtoupper(${$this_is."String"}).'-'.(isset($that_is)?(in_array($this_is.'nom',$that_array_fields)?$editthat[array_search($this_is.'nom',$that_array_fields)]." ".$editthat[array_search($this_is.'prenom',$that_array_fields)]:$editthat[5].($ii>0?$ii:'')):$editthis[5]).'" />-->';
              $content .= '<input type="file" name="'.$this_is.ucfirst($key).'[]" />'.(isset($editthis[$i][$ii])&&$editthis[$i][$ii]!=''?'<a href="'.$mainurl.(stristr($editthis[$i][$ii],$safedir)?$urlsafe.'?file='.base64_encode($editthis[$i][$ii]):$editthis[$i][$ii]).'" target="_new" class="extlink">'.$editthis[$i][$ii].'</a> | <a href="'.$local_url.'&amp;send=delete&amp;'.$this_is.'Id='.$editthis[4].'&amp;'.$this_is.ucfirst($key).'='.$editthis[$i][$ii].'" onclick="return confirm(\''.$confirmationeffacementString.'\');"><img src="'.$mainurl.'images/delete.gif" width="10" height="10" alt="'.$effacerString.'" title="'.$effacerString.'" border="0" /></a>':'').'<br />';
            }
          }
          if ($key == 'img')
          $content .= '</div>';
        ...
        //  $content .= $contentimg.'</div>';
          ...
          if ($editthis[$i]!='') {
            ...
            $content .= '<style>#star_img_coords{position:absolute;z-index:9999;margin-left:'.($star_img_coords[0]>7?$star_img_coords[0]-7:$star_img_coords[0]).'px;margin-top:'.($star_img_coords[1]>7?$star_img_coords[1]-7:$star_img_coords[1]).'px;}</style>';
          }
          $content .= '<label for="'.$this_is.ucfirst($key).'">> '.${$key."String"}.'</label><br /><input name="'.$this_is.ucfirst($key).'" id="'.$this_is.ucfirst($key).'" type="text" value="'.$editthis[$i].'" style="width:97%;" /><br />';
          include '_coords_from_map.php';
        } else if (isset($that_is) && ($key == $old_this))
        $content .= '<label for="'.$this_is.ucfirst($key).'">> '.${$key."String"}.'</label> <!--<br />--><select name="'.$this_is.ucfirst($key).'">'.gen_selectoption($old_dbtable,$editthis[$i],'',$old_this).'</select><br />';
        else if (isset(${"tbl".$key}) && !in_array($key,$basic_array) && !in_array($this_is.$key,$int3_array)) {
          if (in_array($this_is,$editable_by_membre) && !stristr($_SERVER['REQUEST_URI'],$urladmin) && ($logged_in === true) && isset(${$key."Id"}))
          $content .= '';//'<input type="hidden" name="'.$key."Id".'" value="'.${$key."Id"}.'" />';
          else
          $content .= '<label for="'.$this_is.ucfirst($key).'">> '.${$key."String"}.'</label> <!--<br />--><select name="'.$this_is.ucfirst($key).'"'.(in_array($this_is.$key,$int3_interdependant)?' id="'.$this_is.$key.'"'.($this_is.$key==$int3_interdependant[0]?' onchange="changed(this,\''.$int3_interdependant[1].'\')"':' onchange="check_selected(\''.$int3_interdependant[1].'\',\''.$int3_interdependant[0].'\')"'):'').'>'.gen_selectoption(${"tbl$key"},$editthis[$i],(isset($this_id)&&in_array($this_is.$key,$array_uniqueselection)?" AND $key$this_is='$this_id' ":(in_array($this_is.$key,$array_uniqueselection)?" AND ($key$this_is='' OR $key$this_is=NULL)":(isset($send)&&($send=='edit')&&in_array($this_is.$key,$int3_interdependant)&&($this_is.$key==$int3_interdependant[1])?" AND $key".substr($int3_interdependant[0],strlen($this_is))."='".$editthis[$int3_interdependant[0]]."' ":''))).(in_array($key."lang",sql_fields(${"tbl$key"},"array"))?" AND ".$key."lang='$lg' ":''),$key).'</select><br />';
        } else {
          if (in_array("$this_is$key",$mediumtext_array)) {
            $content .= '<label for="'.$this_is.ucfirst($key).'">> '.${$key."String"}.'</label><br /><textarea name="'.$this_is.ucfirst($key).'" rows="20" cols="40" style="width:97%;height:100px;">'.format_edit($editthis[$i],'edit').'</textarea><br />';
          } else if (in_array("$this_is$key",$longtext_array)) {
            $content .= '<label for="'.$this_is.ucfirst($key).'">> '.${$key."String"}.'</label>'.($tinyMCE===false?$text_style_js:'').'<br /><textarea id="elm'.($i_elm>=-1?$i_elm+=1:'').'" name="'.$this_is.ucfirst($key).'" rows="20" cols="40" style="width:97%;height:300px;min-height:300px;">'.format_edit($editthis[$i],'edit').'</textarea><br />';
          } else if (in_array("$this_is$key",$enumYN_array)) {
            $content .= '<label for="'.$this_is.ucfirst($key).'">> '.${$key."String"}.'</label> <!--<br />--><select name="'.$this_is.ucfirst($key).'">'.gen_selectoption(array('N','Y'),$editthis[$i],'','').'</select><br />';
          } else if (in_array("$this_is$key",$enumtype_array)) {
            $content .= '<label for="'.$this_is.ucfirst($key).'">> <b>'.${$key."String"}.'</b></label> <select name="'.$this_is.ucfirst($key).'">'.gen_selectoption($tblenum,$editthis[$i],'',$this_is.$key).'</select>'.(!stristr($_SERVER['REQUEST_URI'],$urladmin)&&($user_can_add_types===false)?'':' <input name="new_'.$this_is.ucfirst($key).'" value="" /> ('.$ajouterString.')').'<br />';
    ...
    if ($moderate === true)
    include '_moderator_captcha.php';
    ...
    if (isset($tblhtaccess) && in_array($this_is,$array_modules) && !in_array($this_is,$array_modules_as_form)) {
      if ($send == 'edit')
      $gethtaccess = sql_get($tblhtaccess,"WHERE htaccessitem='$this_id' AND htaccesstype='$this_is' AND htaccesslang='$lg' ORDER BY htaccessdate DESC ","htaccessmetadesc,htaccessmetakeyw");
      else
      $gethtaccess = array('','');
      if ($gethtaccess[0] == '.') $gethtaccess[0] = '';
      if (!isset($suppress_metas) || ($suppress_metas === false))
      $content .= '<hr style="float:left;" /><div id="offerhtaccess" style="display:block;"><a class="toggle" onclick="javascript:getelbyid(\'htaccessfields\').style.display=\'block\';getelbyid(\'offerhtaccess\').style.display=\'none\';">'.$ajouterString.' '.$metadescString.' &amp; '.$metakeywString.'</a></div><div id="htaccessfields" style="display:none;">'.strtoupper($metadescString.' &amp; '.$metakeywString).'<br /><label for="'.$this_is.'Metadesc"> <b>'.$metadescString.'</b> </label><br /><textarea name="'.$this_is.'Metadesc" rows="3" cols="40" style="width:97%;height:50px;min-height:50px;">'.(isset(${$this_is."Metadesc"})?${$this_is."Metadesc"}:($gethtaccess[0]==''?$desc:$gethtaccess[0])).'</textarea><br /><label for="'.$this_is.'Metakeyw"> <b>'.$metakeywString.'</b> </label><br /><textarea name="'.$this_is.'Metakeyw" rows="3" cols="40" style="width:97%;height:50px;min-height:50px;">'.(isset(${$this_is."Metakeyw"})?${$this_is."Metakeyw"}:($gethtaccess[1]==''?$keyw:$gethtaccess[1])).'</textarea></div>';
    }
    $this_is = $old_this;
    $dbtable = $old_dbtable;
    ...
    $content .= '<br /><input name="send" type="submit" value="'.($send=='new'?$envoyerString:$sauverString).'"> | '.(stristr($_SERVER['REQUEST_URI'],$urladmin)&&(sql_getone($tbladmin,"WHERE adminpriv LIKE '%0%' LIMIT 1 ","adminutil") == $admin_name)&&($send!='new')?'<input name="send" type="submit" value="'.$envoyerString.'"> | ':'').'<input type="reset" value="Reset" /></form></td></tr></table>';
  #######################################################Delete
  } else if ($send == 'delete') {
    if (isset(${$this_is."Id"}) && preg_match("/^[0-9]+\$/",${$this_is."Id"})) {
      ...
      {Header("Location: $redirect");Die();}
        ...
        if (($key != '0') && 
            (($key == $this_is."comment") || (isset($that_is) && ($key == $that_is."comment")) ||
            (($key == $this_is."doc") || (isset($that_is) && ($key == $that_is."doc")) ||
             ($key == $this_is."img") || (isset($that_is) && ($key == $that_is."img")))
            )) {
          if ($value != "") {
            $deletequery = 0;
            if (($key == $this_is."comment") || (isset($that_is) && ($key == $that_is."comment")))
      ...
      if (isset(${$this_is."Img"}) || isset(${$this_is."Doc"}) ||
        ...
        if ($error == '') {
          $_SESSION['mv_notice'] = $notice;
          if (in_array($this_is,$editable_by_membre) && !stristr($_SERVER['REQUEST_URI'],$urladmin) && ($logged_in === true) && isset(${$this_is."Id"}))
