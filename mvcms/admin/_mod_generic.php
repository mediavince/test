...
if (($logged_in === true) && isset($send) && in_array($this_is,$editable_by_membre) && (lgx2readable($lg,$x) == lgx2readable($lg,'',$this_is)) && ($admin_viewing === false)) {
  $edit_text = true;
  include $getcwd.$urladmin.'itemadmin.php';
} else {
...
if ((isset($rqst) || isset($toggle)) && isset(${$this_is."Id"})) {
    ...
    if ($this_is == 'admin') {
    //  if (sql_getone($dbtable,"WHERE adminid='".${$this_is."Id"}."' AND adminlang='$lg' ","adminutil") == $admin_name) {
    ...
    if ($error=='') {
      foreach($array_fields as $k_check) {
        ...
        if (isset(${$field_check."Id"})) {
          ...
          $notice .= '<b>'.$enregistrementString.' '.$pourString.' '.$field_check.' '.$class_conjugaison->plural((mv_toggle(${"tbl".$field_check},$field_check.(in_array($field_check.$toggle,$array_fields)?$toggle:"statut")," ".$field_check.(in_array($field_check."rid",sql_fields(${"tbl".$field_check},'array'))?'r':'')."id='".${$field_check."Id"}."' AND ".$field_check.$this_is."='".${$this_is."Id"}."' ")=='Y'?$confirmeString:$enattenteString),'M',1).'</b>';
    ...
    Header("Location: ".html_entity_decode($local_url.'&amp;'.$this_is.'Id='.${$this_is."Id"}.(isset(${$field_check."Id"})?'&amp;'.$field_check.'Id='.${$field_check."Id"}:'').(isset($send)?'&amp;send='.$send:'')));Die();
 	} else
  {Header("Location: $redirect");Die();}
}
  ...
  //echo ${$this_is."Id"}." $dbtable,"."WHERE ".$this_is."id='".${$this_is."Id"}."' ";
    ...
    foreach($show_array_by_id as $key) {
      if (strstr($key,":")) {
        ...
        ${"show_".$key[0]."_".$key[1]} = sql_getone(${"tbl".$key[0]}," WHERE ".$key[0].(in_array($key[0]."rid",sql_fields(${"tbl".$key[0]},'array'))?"lang='$lg' AND ".$key[0]."r":'')."id='$show_id' ",$key[0].$key[1]);
        else if ($key[1] == 'cut') {
          ${"show_".$key[0]} = substr(strip_tags(sql_getone(${"tbl".$this_is}," WHERE ".$this_is.(in_array($this_is."rid",sql_fields(${"tbl".$this_is},'array'))?"lang='$lg' AND ".$this_is."r":'')."id='$show_id' ",$this_is.$key[0])),0,300);
        } else {
          ${"show_".$key[0]."_".$key[1]} = '';
          if (isset(${"show_".$key[0]."_".$key[1]."_array_linked_by_id"})) {
            $loop = sql_array(${"tbl".$key[0]}," WHERE ".$key[0].$this_is."='$show_id' AND ".$key[0].$key[1]."='Y' AND ".$key[0]."lang='$lg' ",$key[0]."rid");
            if ($loop[0]!='')
            foreach($loop as $rel_id){
              $getitem = sql_get(${"tbl".$key[0]}," WHERE ".$key[0]."rid='$rel_id' AND ".$key[0]."lang='$lg' ",$key[0]."rid,".$key[0].implode(",".$key[0],${"show_".$key[0]."_".$key[1]."_array_linked_by_id"}));
              if ($getitem[0]!='.') {
                $link_id = $getitem[0];
                for($i=0;$i<count(${"show_".$key[0]."_".$key[1]."_array_linked_by_id"});$i++) {
                ${"link_".${"show_".$key[0]."_".$key[1]."_array_linked_by_id"}[$i]} = (${"show_".$key[0]."_".$key[1]."_array_linked_by_id"}[$i]=='gendre'?sql_stringit("gendre",$getitem[$i+1]):$getitem[$i+1]);//there is the id as getitem[0] 
                }
  ...
  //  if (!isset(${$this_is."Id"}) && (!isset(${"filter_search"}) || (isset(${"filter_search"}) && ((${"filter_search"}===true) || in_array($this_is.${"filter_search"},$array_fields) || (isset($that_is) && in_array($that_is.${"filter_search"},$that_array_fields))))))
    ...
    $_head_mod_content .= ($admin_viewing===true?gen_form($lg,$x,$y):gen_form($lg,$x)).'<div style="float:right;">'.$_pre_css_bgimg.'<div class="bgimg-two-bot-search"><input type="text" name="q" value="'.(isset($q)?$q:'').'" /><input type="submit" value="'.$rechercherString.'" />'.(isset($q)&&($q_count>0)?'<br /> > '.$q_count.' '.$class_conjugaison->plural($confirmeString,'',$q_count):'').' ('.(isset($filter_searchfield)?${$filter_searchfield."String"}:${(substr($array_fields[5],strlen($this_is))=='gendre'?'nom':substr($array_fields[5],strlen($this_is)))."String"}).')</div>'.$_post_css_bgimg.'</div></form>';
    $_head_mod_content .= '</div>';
    if (in_array($this_is."coords",$array_fields)&&!isset($that_is))
    $_ordered_mod_content = $_pre_css_bgimg.'<a href="'.($admin_viewing===true?$local.'?lg='.$lg.'&amp;x='.(isset($filter_mapdisplay)&&preg_match("/^[0-9]+\$/",$filter_mapdisplay)?$filter_mapdisplay:$x):lgx2readable($lg,(isset($filter_mapdisplay)&&preg_match("/^[0-9]+\$/",$filter_mapdisplay)?$filter_mapdisplay:''))).'">&nbsp;'.$mapString.'&nbsp;</a>'.$_post_css_bgimg.(isset($_ordered_mod_content)?$_ordered_mod_content:'');
...
//reset
foreach(array($this_is."Id",$this_is."Type","protected_show","view_see_all_button","lightbox","show_oneimg_only","q","this_is","filter_searchfield","list_array_fields","that_is","send","filter_map","filter_index","filter_future","filter_past","filter_ordre","filter_leftlist","rqst","toggle","future","past","ordre","par","index_by_alpha","ordered_by","this_index","past_index","show_array","array_tpl","show_array_by_id","array_tpl_by_id","show_array_linked","array_tpl_linked","linked_content","bypass_this","select_array","filter_mapdisplay") as $reset_k)
${$reset_k} = NULL;
