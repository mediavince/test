...
$forgotpasshelpString = '<b>Follow these instructions:</b><br />> Input your username<br />> Check that box<br />> Click login<br />A new password will be sent to the email address on file!';
$div_forgotpass = '<div id="forgotpasshelp" class="helpbox">'.$forgotpasshelpString.'</div>';
if (!isset($nouveaumdpachangerString))
$nouveaumdpachangerString = 'Your new password has been sent to your email address, please update it upon your next login!';

$get_profil = sql_get($tblcont,"WHERE contlang='$lg' AND conttype='profil' ","conturl,conttitle,contpg");
$url_profil = $mainurl.$get_profil[0];
$get_inscrire = sql_get($tblcont,"WHERE contlang='$lg' AND conttype='inscrire' ","conturl,conttitle,contpg");
$url_inscrire = $mainurl.$get_inscrire[0];
if ($htaccess4sef === true) {
  $url_profil = $mainurl.lgx2readable($lg,$get_profil[2]);//(count($array_lang)>1?"$lg/":'').space2underscore($get_profil[1]);
  $url_inscrire = $mainurl.lgx2readable($lg,$get_inscrire[2]);
}

$form_login = '<input type="hidden" name="redirURL" value="'.substr($local_uri,1).'" /><label for="userName"> '.$nomutilString.' </label> <input class="inputtext" name="userName" type="text" autocomplete="off" /><br /><label for="passWord"> '.$motdepasseString.' </label> <input class="inputtext" name="passWord" type="password" autocomplete="off" /><br /><label for="forgotpass"> '.$forgotpassString.' </label> <input name="forgotpass" type="checkbox" id="forgotpasscheckbox" /> <img src="'.$mainurl.'images/help.gif" onmouseover="javascript:getelbyid(\'forgotpasshelp\').style.display=\'block\';" onmouseout="javascript:getelbyid(\'forgotpasshelp\').style.display=\'none\';" onclick="javascript:getelbyid(\'forgotpasscheckbox\').checked=\'true\';" border="0" /><br />'.$div_forgotpass.'<input name="login" type="submit" value="'.$entrerString.'" />'.($get_inscrire[0]!='.'?' <a href="'.$url_inscrire.'">'.$get_inscrire[1].'</a>':'').'</form>';

$form_error = ' * '.$accesString.' '.$refuseString.' * ';

$form_logout = '<input type="hidden" name="redirURL" value="'.substr($local_uri,1).'" /><input name="login" type="submit" value="'.$sortirString.'" /> <a href="'.$url_profil.'">'.$get_profil[1].'</a></form>';
...
//		$user_idurl = $read_cookie[3];
    ...
    $read = sql_get($tbluser,"WHERE userutil='$user_name' ","user".(in_array("userrid",$user_array_fields)?"r":'')."id,useremail");
    $user_id = $read[0];
    $user_email = $read[1];
		if (sql_nrows($dbtable," WHERE userutil='$user_name' AND userpass='$pass_word' $sql_priv AND userstatut='Y' ") > 0) {// == (in_array("userrid",$user_array_fields)?count($array_lang):'1')
	  	$loginform = gen_form($lg).$user_name.'<br />'.$form_logout;
  	  $logged_in = true;
      if ((md5(sql_getone($tbluser,"WHERE userutil='$user_name' ","useremail")) == $pass_word) && !stristr($url_profil,$_SERVER["REQUEST_URI"])) {
...
#####################################
  ...
  //  $this_isPass = substr(md5(substr(md5(microtime() * mktime()),0,rand(5,8))),0,rand(6,9));
    $this_isPass = substr(base64_encode(md5(microtime() * mktime())),0,rand(6,9));
		$read = sql_update($dbtable,"SET userpass='".md5($this_isPass)."' "," WHERE userutil='$user_name' AND userstatut='Y' ","userutil,useremail,userpriv");
		// SEND RESET PASSWORD
		...
		if (!$connection) {
  		...
  		$error .= $form_error;
			...
			$read = sql_get($dbtable,"WHERE userutil='$user_name' AND userpass='$pass_word_md5' AND userstatut='Y' ","userutil,userpass,userdate,userpriv,useremail,user".(in_array("userrid",$user_array_fields)?"r":'')."id");
			$user_email = $read[4];
			$user_id = $read[5];
			$user_priv = explode("|",$read[3]);
			if (!is_array($user_priv)) $user_priv = array($user_priv);
			$countread = sql_nrows($dbtable, " WHERE userutil='$user_name' AND userpass='$pass_word_md5' AND userstatut='Y' ");
			if ( (!$read) || ($read[0] == '.') || ($countread == 0) ) {// ($countread != (in_array("userrid",$user_array_fields)?count($array_lang):'1'))
				$loginform = gen_form($lg).$form_login;//.$form_error
				$logged_in = false;
    		$error .= $form_error;
			} else {
			// if connection succeeded get info from DB and compare variables
      		...
      		$error .= $form_error;
					...
					setcookie($cookie_codename."user","$info",0);//,'/'.$urlclient ???last double quote needed for assuring persistency of login // checked and not needed as long as action goes root
					$update = sql_update($dbtable,"SET userdate=$dbtime","WHERE userutil='$user_name' AND userpass='$pass_word_md5' ", "userdate");
					$loginform = $bienvenueString.' '.$user_name.' !<br /><sup>'.$dernierString.' '.$accesString.' : "'.$read[2].'"</sup><br />';
				  $logged_in = true	;
          $notice = ''.$loginform;//.'<div style="background-color: #CF9;border: 1px solid green;padding: 5px;margin: 5px;text-align:center;"></div>';
					$loginform = gen_form($lg).$loginform.$form_logout;
					if ($read[1] == md5($read[4])) {
						$pwdtochange = true;
            $notice = '<div style="background-color: #FA5;width: 90%;border: 1px solid red;padding: 5px;margin: 5px;text-align:center;">'.strtoupper($modifierString.' '.$motdepasseString).' !'.(!stristr($_SERVER["PHP_SELF"],$get_profil[0])?' >> <a href="'.$url_profil.'">'.$get_profil[1].'</a>':'').'</div>';
          }
		  $_SESSION['mv_notice'] = $notice;
		  Header("Location: $redirect".(isset($redirURL)?$redirURL:''));Die();
				}
