...
function format_edit($text,$do)
{
    ...
    if ($tinyMCE === false) {
        if ($do == "edit") {
            ...
            $text = \preg_replace('" border="0" \/>','[/img]',$text);

            $text = \preg_replace('<img width="([^]]*)" height="([^]]*)" src="([^]]*)" align="right" hspace="5" vspace="5" alt="([^]]*)','[img=\3]\4',$text);

            $text = \preg_replace('</div>','[/div]',$text);
            $text = \preg_replace('" id="div">',']',$text);
            $text = \preg_replace('<div style="width:([^]]*);text-align:([^]]*);float:([^]]*);','[div=\1,\2,\3',$text);

            $text = \preg_replace('</a>','[/url]',$text);

            $text = \preg_replace('<a href="([^]]*)" target="_blank" name="doc">([^]]*)','[url=doc:\1]\2',$text);
            $text = \preg_replace('<a href="'.$mainurl.'([^]]*)" target="_self">([^]]*)','[url='.$mainurl.'\1]\2',$text);
            $text = \preg_replace('<a href="([^]]*)" target="_blank">([^]]*)','[url=\1]\2',$text);

            $text = \preg_replace('<i>','[i]',$text);
            $text = \preg_replace('</i>','[/i]',$text);
            if (in_array($lg, array('es','fr','it'))) {
                $text = \preg_replace('<b>','[g]',$text);
                $text = \preg_replace('</b>','[/g]',$text);
            ...
            $text = \preg_replace('\[email=mailto:]\[/email]','',$text);
            $text = \preg_replace('\[email=]\[/email]','',$text);
            $text = \preg_replace('\[url=doc://]\[/url]','',$text);
            $text = \preg_replace('\[url=https://]\[/url]','',$text);
            $text = \preg_replace('\[url=https://]\[/url]','',$text);
            $text = \preg_replace('\[url=mailto:]\[/url]','',$text);
            $text = \preg_replace('\[url=]\[/url]','',$text);
            $text = \preg_replace('\[div=]','',$text);
            $text = \preg_replace('\[div=]\[/div]','',$text);
            $text = \preg_replace('\[img=]','',$text);
            $text = \preg_replace('\[img=][/img]','',$text);
            $text = \preg_replace('\[flash=]','',$text);
            $text = \preg_replace('\[i]\[/i]','',$text);
            ...
            $text = \preg_replace('\[img=https://([^]]*)]([^]]*)\[/img]','<img src="https://\1" align="right" hspace="5" vspace="5" alt="\2" border="0" \/>',$text);
            $text = \preg_replace('\[img=([^]]*)]([^]]*)\[/img]','<img src="\1" align="right" hspace="5" vspace="5" alt="\2" border="0" \/>',$text);

            $text = \preg_replace('\[div=([^]]*),([^]]*),([^]]*)]','<div style="width:\1;text-align:\2;float:\3;" id="div">',$text);

            /* error with $up inserted prior content/... for admin */
            //	$text = \preg_replace('\[url=doc:([^]]*)]\[/url]','<a href="'.$mainurl.'\1" target="_blank" name="doc">'.$cliquericiString.'</a>',$text);
            //	$text = \preg_replace('\[url=doc:([^]]*)]([^]]*)\[/url]','<a href="'.$mainurl.'\1" target="_blank" name="doc">\2</a>',$text);
            $text = \preg_replace('\[url=doc:([^]]*)]\[/url]','<a href="\1" target="_blank" name="doc">'.$cliquericiString.'</a>',$text);
            $text = \preg_replace('\[url=doc:([^]]*)]([^]]*)\[/url]','<a href="\1" target="_blank" name="doc">\2</a>',$text);
            ...
            $text = \preg_replace('\[url='.$mainurl.'([^]]*)]([^]]*)\[/url]','<a href="'.$mainurl.'\1" target="_self">\2</a>',$text);
