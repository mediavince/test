...
function gen_form($lg,$x=NULL,$y=NULL) {
  ...
  if (isset($x)&&!isset($y))
  return '<form enctype="multipart/form-data" name="f'.$x.'" action="'.$local.($htaccess4sef===true?lgx2readable($lg,$x):$indexdotphpfroform.'?lg='.$lg.'&amp;x='.$x).'" method="'.$postgetmethod.'">';//$form_method
  else if (isset($y))
  return '<form enctype="multipart/form-data" name="f'.$x.'" action="'.$local.$indexdotphpfroform.'?lg='.$lg.'&amp;x='.$x.'&amp;y='.$y.'" method="'.$postgetmethod.'">';//$form_methody
  else
  return '<form enctype="multipart/form-data" name="f" action="'.$local.(!stristr($local,$urladmin)?($htaccess4sef===true?$lg:'?lg='.$lg):'').'" method="'.$postgetmethod.'">';//$form_method
 // return '<form enctype="multipart/form-data" name="'.$x.'" action="" method="'.$postgetmethod.'">';//$form_method_self // does not work on chrome... returns to root...
}
...
function human_date($this,$hour=NULL,$class=NULL){
  ...
  if (strstr($this,"-")) {
    ...
    if (('13'>$month) && ($month>'0')) {
      ...
      if ($class!=NULL)
      $output = '<span'.($class=='nl'?'><img src="'.$mainurl.'images/cal_sml.png" align="left" width="17" height="20" alt=" " border="0" /> ':' class="cal_sml">').$output.'</span>';
    }
  }
return $output;
}
...
function gen_lang() {
	global $trace, $mainurl, $local, $urladmin, $url_mirror, $extractfromphpself, $x, $y, $lg, $tblcont, $tblenum, $tblstring, $default_lg, $htaccess4sef, $local_uri, $tblhtaccess, $array_lang, $x1subok;
  ...
  //  $local_uri = "/".$default_lg.$local_uri;
  	foreach($array_lang as $key) {
  		...
  		if	($key == $lg)	${"tabinverted".$key} = $tabinverted	;
  		else	${"tabinverted".$key} = ""	;
  		...
  		if	(stristr($_SERVER["PHP_SELF"],$urladmin))	$gen_lang .= $local.$params.'lg='.$key.'"'	;
        	...
        	$redir_uri = $key;
    	      ...
    	      for(($x1subok===true?$hj=1:$hj=2);$hj<strlen($x);$hj++){
    	        ...
    	        $redir_uri .= "/".space2underscore(sql_getone($tblcont,"WHERE contlang='$key' AND contpg='$substr_pg' ","conttitle"));
    	      }
    	    }
        	...
        	$redir_uri = lgx2readable($key,'',$field,$item);
  			...
  			if	($params == "?")	$params = ""	;
  			if	($x == '1')	 $gen_lang .= ($htaccess4sef===true?$url_mirror.$redir_uri:'index_'.$key.'.php').$params.'"'	;
  			else	$gen_lang .= ($htaccess4sef===true?$url_mirror.$redir_uri:substr($this_extracted_url,0,-7).'_'.$key.'.php').$params.'"'	;
  		}
  		...
  		$gen_lang .= '<img src="'.$mainurl.'images/'.$key.'.gif" '.$aligntoleft.' title="'.$sql_get_lang.'" alt="'.$sql_get_lang.'" border="0" width="24" height="24" class="'.${"tabinverted".$key}.'" /></a>&nbsp;';
  	//	$gen_lang .= '<img src="'.$mainurl.'images/tab'.${"tabinverted".$key}.'_'.$key.'.gif" id="tab_'.$key.'" '.$aligntoleft.' alt="'.sql_getone($tblstring,"WHERE stringlang='$key' AND stringtype='lang' AND stringtitle='$keyid' ","stringentry").'" border="0" /></a>&nbsp;';
  	}
  }
return $gen_lang;
}
...
function sql_getone($dbtable,$where,$getone) {
	$sql = @mysql_query("SELECT $getone FROM $dbtable $where");
	$row = @mysql_fetch_array($sql);
return $row[0];
}

function lgx2readable($this_lg,$this_x,$mod=NULL,$id=NULL) {
	global $trace,$x,$notice,$admin_viewing,$array_lang,$tblcont,$x1subok,$full_url,$htaccess4sef,$tblhtaccess,$this_is,$filter_index,$filter_map,$filter_archive,$ordered_by,${"tbl".$this_is};
	$redirtourl = '';
    ...
    if ($admin_viewing === true) {
      ...
      if (isset($mod)) {
        if (isset($id)) {
          $this_type = sql_getone(${"tbl".$this_is},"WHERE ".$this_is."id='$id' ",$this_is."type");
          $this_archive = sql_getone(${"tbl".$this_is},"WHERE ".$this_is."id='$id' ",$this_is."archive");
          if ($this_type != '')
  ...
  //	if (($full_url === true) && (($mod !== NULL) && ($id !== NULL))) {
      if (!isset($filter_archive) && (isset($ordered_by) && strstr($ordered_by,'type'))) {
        $type_url = space2underscore(sql_stringit($this_is.'type',sql_getone(${"tbl".$this_is},"WHERE ".$this_is."rid='$id' ",$this_is."type")));
      }
    ...
    } else if (isset($this_lg) && ($this_x == '') && ($mod !== NULL)) { // compares against type
...
function sql_update($dbtable,$setq,$where,$getrow) {
	$sql = @mysql_query("UPDATE $dbtable $setq $where");
	$sql = @mysql_query("SELECT $getrow FROM $dbtable $where ");
	$row = @mysql_fetch_array($sql);
	if (is_array($row)) {
...
function sql_nrows($dbtable,$where) {
	$sql = @mysql_query("SELECT * FROM $dbtable $where");
...
function sql_del($dbtable,$where) {
	$sql = @mysql_query("DELETE FROM $dbtable $where");
	$sql = @mysql_query("SELECT * FROM $dbtable $where");
	$row = @mysql_num_rows($sql);
return $row;
}
...
global ${"filter_".$key_filter};
    if (isset(${"filter_".$key_filter}) && !in_array($this.$key_filter,$datetime_array)) {
      if (is_bool(${"filter_".$key_filter}) && in_array($this.$key_filter,$enumYN_array))
      $sql_q .= " AND $this$key_filter='".(${"filter_".$key_filter}===true?'Y':'N')."' ";
      else
      $sql_q .= " AND $this$key_filter='".${"filter_".$key_filter}."' ";
    } else {
...
function sql_nemails($email) { //!!!	needs sql_fields and is_email above	//
	...
	$sql = @mysql_query("SELECT * FROM $select WHERE $where");
	$row = @mysql_num_rows($sql);
return $row;
}
...
################# ### upl img square size of side > max_sqrt corresponding to servers -> error
		if ($sqrt_tmp > $max_sqrt) {
			...
			if ($width_orig >= $height_orig) {// ############## LANDSCAPE OR SQUARE
			...
			} else if ($height_orig > $width_orig) {// ############## PORTRAIT
...
function gen_selectoption($table,$selected,$type,$what,$show_count=NULL) {
    ...
    foreach($table as $key) {
      ...
      $selectoptions .= '<option value="'.$key.'"'.$this_select.'>'.(sql_stringit(($what!=''?$what:'general'),$key)!=''?sql_stringit(($what!=''?$what:'general'),$key):(${$key."String"}==''?$key:${$key."String"})).'</option>';
    }
...
class Conjugaison {
	...
	function plural($text,$genre,$count) {
		global $lg;
		if ($count > '1') {
			if ($lg == 'fr') { // puts s at every words end
				$s = "s"	;
				if	($genre == 'F')	$fem = "e"	;
				if	($genre == 'M')	$fem = ""	;
				if ($genre == '') {
			...
			} else if ($lg == 'en') { // puts s only on last words end
				$s = "s";
				$fem = "";
				if (strstr(trim($text),' ')) {
          ...
          $array_inv = array('Dear');
			...
			} else if (($lg == 'it') || ($lg == 'es')) { // puts i or e at every words end according to genre M or F
				$s = ""	;
				$fem = "";
				if	($genre == 'M')	$s = "i"	;
				if	($genre == 'F')	$s = "e"	;
    ...
    function extractTPLZip($src,$dest) {
      ...
      if (function_exists('zip_open')) {
        ...
        function mkdirr($dest,$mode=null) {
          ...
          if(is_file($dest)) {trigger_error('mkdirr() File exists', E_USER_WARNING);return false;}
          $next_pathname=substr($dest,0,strrpos($dest,DIRECTORY_SEPARATOR));
          if(mkdirr($next_pathname,$mode)) {if(!file_exists($dest)) {return mkdir($dest,$mode);} }
          return false;
        }
        unzip($src,$dest);
      } else {
