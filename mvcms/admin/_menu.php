...
if ($logged_in === true) {
	...
	if (isset($x) && ($x == 'z') && in_array('1',$admin_priv)
		&& ((!isset($adminId) || (isset($adminId) && ($adminId != $admin_id))) && ($y != '2')
		&& (!isset($send) || (isset($send) && (($send != 'edit') || ($send != $sauverString)))
	)))
		//	&& ($_SERVER['QUERY_STRING'] != $admin_profil_url)
	{Header("Location: $mainurl$urladmin");Die();}
}

	if (isset($_POST['login']) && (!isset($_GET['x']) || !isset($_POST['x'])))
	//	&& ($login == $entrerString))
...
$lg = $default_lg;
$pgcontent = sql_get($tblcont,"WHERE contpg='$x' AND contlang='$lg' ",
...
if ($logged_in === true) {
	...
	if ($x == 'z') {
		...
		if (!isset($y) || (!preg_match("/^[0-9]+\$/",$y)
							&& !in_array($y,$array_keys_admin_menu))
			|| (preg_match("/^[0-9]+\$/",$y) && (($y<1)
			|| (($y>8) && (sql_getone($tbladmin,
					"WHERE adminpriv LIKE '%0%' LIMIT 1 ","adminutil") != $admin_name))
			|| (($y>13)	&& (sql_getone($tbladmin,
					"WHERE adminpriv LIKE '%0%' LIMIT 1 ","adminutil") == $admin_name))))
		)
		$y = "1";
		$i = 0;
		foreach($array_admin_menu as $key=>$ico) {
			if (in_array($ico,$array_super_admin_menu)) { // needs 2dim array
				...
				$loginform .= '<a style="padding:2px;" href="?lg='.$lg.'&amp;x=z&amp;y='
							...
							.'" border="'.($y==$key?'1" style="border:1px solid;padding:2px;':
							'0" style="padding:4px;').'" /></a>';
							//<div style="float:left;"></div>
			} else {
				if (in_array($ico,$array_basic_admin_menu)
					|| (in_array($ico,$array_modules) && (${"mod_".$ico} === true))) {
					if (in_array($ico,$array_modules)) {
						// if array is not 2dim defaults to categories
					...
					if ($key != 'membre') {
						if (($key == 'user') && in_array('membre',$array_modules)) {
							// if array is not 2dim defaults to categories
							$key = 'membre';
						}
			            /*
			      		  ...
			      		  .'" border="'.($y==$key?'1" style="border:1px solid;padding:2px;':
			      		  '0').'" /></a></div><div style="float:left;text-align:left;padding:
			      		  2px;"><b>'.${$key."String"}.'</b><br />informations</div></div>';
			      		  //<hr /><br />
			      		  */
						if (($y == '1') && !in_array('0',$admin_priv)) {
							...
							$admin_menu .= '<div style="float:left;min-height:150px;width:23%;
								...
								align="left" border="0" /></a></div><b>'.ucfirst(${$key
								."String"}).'</b><br />'.(isset(${$key."HelpString"})?${$key
								."HelpString"}:'').'</div>';
					//<hr /><br /><div style="float:left;text-align:left;padding:2px;"></div>
						...
						$admin_menu .= '<div style="float:left;max-width:100px;"><a
							...
							.'" alt="'.$gestionString.' '.$surString.' '.${$key."String"}
							.'" border="'.($y==$key?'1" style="border:1px solid;padding:2px;':
							'0').'" '.(isset(${$key."HelpString"})?' onmouseover="javascript:'
							.'getelbyid(\'help'.$key.'\').style.display=\'block\';"
							onmouseout="javascript:getelbyid(\'help'.$key
							.'\').style.display=\'none\';"':'').' /><br />'.ucfirst(${$key
							."String"}).'</a>'.(isset(${$key."HelpString"})?'<div id="help'
							.$key.'" class="helpbox" style="display:none;float:right;">'
							.${$key."HelpString"}.'</div>':'').'</div>';
					}
					$i++;
				}
			}
		...
		if ($y == '1') include $getcwd.$up.$urladmin.'extraoptions.php';
		if (($y == '2') || ($y == 'admin')) {
		...
		if (($y == '3') || ($y == 'user') || ($y == 'membre')) {
			...
			if ($y == 'membre') {
				...
				$_membre_array_mandatory_fields = array('','gendre','nom','prenom',
														'profession');
				$that_is = 'membre';
			}
			include $getcwd.$up.$urladmin.'itemadmin.php';
		}
		if (($y == 'communications')) include $getcwd.$up.$urladmin.'communicationsadmin.php';
		if (($y == '4') || ($y == 'newsletter')) include $getcwd.$up.$urladmin.'newsletteradmin.php';
		if (($y == '5') || ($y == 'string')) include $getcwd.$up.$urladmin.'textesadmin.php';
		if (($y == '6') || ($y == 'bannedip')) include $getcwd.$up.$urladmin.'bannedipsadmin.php';
		if (($y == '7') || ($y == 'media')) include $getcwd.$up.$urladmin.'mediaadmin.php';

		if (in_array($y,$array_modules_admin)) {
			$this_is = $y;
			if (@file_exists($getcwd.$up.$urladmin.$this_is.'admin.php'))
			include $getcwd.$up.$urladmin.$this_is.'admin.php';
			else
			include $getcwd.$up.$urladmin.'itemadmin.php';
		}
		...
		if (($y == '13') || ($y == 'config')) include $getcwd.$up.$urladmin.'configadmin.php';
		if (($y == '14') || ($y == 'module')) include $getcwd.$up.$urladmin.'moduleadmin.php';
	} else {
