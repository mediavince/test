...
if (!stristr($_SERVER['PHP_SELF'],$urladmin)) {
    ...
    if (isset($_GET['item']) && is_int($_GET['item']+0))
        $cartitem = $_GET['item'];
    if (isset($_GET['add-to-cart']))
        $addtocart = $_GET['add-to-cart'];
    if (isset($_GET['remove-from-cart']))
        $removefromcart = $_GET['remove-from-cart'];
    ...
    } else $leftlinksentry .= $previewcart;
}
