    ...
    foreach($array_fields as $key) {
        ...
        if (isset($q) && ($q != '')) {
            if (($key != $this_is."id") && ($key != $this_is."date") && ($key != $this_is."statut")) {
                $array_q = explode(" ",$q);
                if (!isset($array_q[1]))
                    $array_q = ($sql_q!=''?" OR ":'').$key." LIKE '%".$q."%' ";
                else
                    $array_q = implode("%' OR ".$key." LIKE '%",$array_q);
                if ($array_q != '')
                    $sql_q .= $array_q;
            }
        }
...
if ( isset($send) && (($send == $sauverString) || ($send == $envoyerString)) && isset($_SESSION['mail_count']) && isset($_SESSION['profil_key']) ) {// ($send == 'delete') || ($send == $vousinscrireString) ||
    ...
    if (($tabbing === false) || (($tabbing === true) && ($array_tabs[substr($do,-1)]=="user"))) {// && ($do == 'tabcontent0')
        ...
        if (($email !== '') && is_email($email))
            $email = strip_tags(html_encode($email)) ;
        else
            $error_report .= $emailString.' > '.$error_invmiss.'<br />' ;
        ...
        if ($send == $sauverString) {
            if (($tabbing === false) || (($tabbing === true) && ($array_tabs[substr($do,-1)]=="membre"))) {// && ($do == 'tabcontent1')
  			...
  			if ($updateq[0] == '.') {
                ...
                } else $notice .= '<font color="Green"><b>'.$modificationString.' '.$class_conjugaison->plural($effectueString,'F','1').'!</b></font>';
  			}
  		} else {
...
if ($tabbing === true) {
    ...
    } else $_mod_content .= gen_form($lg,$x);
    ...
    } else $_mod_content .= $show_mod_content;
}
$_mod_content .= ' </div><br />';
if ($tabbing === true) {
    foreach($array_tabs as $key=>$value)
        ...
        } else $_mod_content .= '<div id="tcontent'.$key.'" class="profiltabcontent">'.${"tab_".$array_tabs[$key]}.'</div>';
        /*
      ...
      //	$_mod_content .= '<input name="send" type="submit" value="'.($logged_in===true?$modifierString:$vousinscrireString).'" /><input type="hidden" name="do" value="tabcontent2" /></form>';
        ...
        */
    	...
    	$_mod_content .= '<script type="text/javascript">initializetabcontent("profilmaintab")</script>';
