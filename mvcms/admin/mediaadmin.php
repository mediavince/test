...
if ($logged_in === true) {
  ...
  if (!isset($send) || (isset($send) && ($send == 'edit') && ($y != 'media'))) {
		$delete_media = "";
// ########################## PICS ###############################
		$contPhotocount = sql_nrows($tblcontphoto," ".(in_array("contphotolang",$photo_fields)?" WHERE contphotolang='$lg' ":'')); // WHERE contphotocontid='$x' 
		if (($tinyMCE === false) && (isset($send) && ($send == 'edit') && ($y != 'media'))) {
			if ($contPhotocount > '0') {
				$photoListe = '<a href="'.$local_url.'&amp;send=upldphoto">'.$ajouterString.' 1 '.$photoString.'</a><hr />';
				$photoListe .= $contPhotocount.' '.$class_conjugaison->plural($photodispoString,'F',$contPhotocount).'.<br />';
				$read = @mysql_query("SELECT * FROM $tblcontphoto ".(in_array("contphotolang",$photo_fields)?" WHERE contphotolang='$lg' ":''));
				...
				for ($i=0;$i<$contPhotocount;$i++) {
  				...
  				if (in_array($ext,$array_swf_ext)) {
						$photoListe .= '<a href="#" onclick="insertFlash(\''.$row["contphotoimg"].'\');return(false)" title="'.$row["contphotodesc"].'" alt="'.$row["contphotodesc"].'"><sup>'.str_replace('_',' ',$row["contphotoimg"]).'</sup></a> | <a href="'.$local_url.'&amp;send=delphoto&amp;contphotoId='.(isset($row["contphotorid"])?$row["contphotorid"]:$row["contphotoid"]).'" onclick="return confirm(\''.$confirmationeffacementString.'\');"><img src="'.$mainurl.'images/delete.gif" width="10" height="10" title="'.$effacerString.'" alt="'.$effacerString.'" border="0" /></a><br />';
					} else { // ($ext == "gif") || ($ext == "jpg") || ($ext == "jpeg") || ($ext == "png") || //
						$photoListe .= '<a href="#" onclick="insertImage(\''.$row["contphotoimg"].'\');return(false)" title="'.$row["contphotodesc"].'" alt="'.$row["contphotodesc"].'"><img src="'.$mainurl.$row["contphotoimg"].'" width="50" height="50" hspace="5" vspace="5" border="0" alt="'.$row["contphotodesc"].'" /></a> <a href="'.$local_url.'&amp;send=delphoto&amp;contphotoId='.(isset($row["contphotorid"])?$row["contphotorid"]:$row["contphotoid"]).'" onclick="return confirm(\''.$confirmationeffacementString.'\');"><img src="'.$mainurl.'images/delete.gif" width="10" height="10" title="'.$effacerString.'" alt="'.$effacerString.'" border="0" /></a><br />';
					}
				}
				if	($contPhotocount > 3)	$photoListe .= '</div>';
			} else {
				$photoListe = '<a href="'.$local_url.'&amp;send=upldphoto">'.$ajouterString.' 1 '.$photoString.'</a><hr />'.$pasdeString.' '.$photodispoString.' !<br />';
			}
		} else { // $tinyMCE === true
			$photoListe = '<div><div style="float:left;width:49%;text-align:right;"><a href="'.$local_url.'&amp;send=upldphoto">'.$ajouterString.' 1 '.$photoString.'</a> |</div>'; // write javascript for import in tinymce.
      if (isset($send) && ($send == 'edit') && ($y != 'media'))
			$tinyMCE_photos = '<div style="float:left;overflow:auto;height:110px;width:49%;min-width:300px;">';
			else
			$tinyMCE_photos = '<div style="float:left;overflow:auto;height:300px;width:49%;min-width:300px;">';
			...
			for ($i=0;$i<$contPhotocount;$i++) {
				...
				$ext = strtolower(strrev($ext[0]));
				if (in_array($ext,$array_swf_ext)) {
					$loop_tinyMCE_flashs .= '<a href="'.$local_url.'&amp;send=delphoto&amp;contphotoId='.(isset($row["contphotorid"])?$row["contphotorid"]:$row["contphotoid"]).'" onclick="return confirm(\''.$confirmationeffacementString.'\');"><img src="'.$mainurl.'images/delete.gif" width="10" height="10" title="'.$effacerString.'" alt="'.$effacerString.'" border="0" /></a> | <a href="'.$mainurl.$row["contphotoimg"].'" target="_blank"><img src="'.$mainurl.'images/'.$ext.'logo.gif" width="16" height="16" border="0" title="'.$ouvrirString.' '.$ext.'" alt="'.$ouvrirString.' '.$ext.'" /> '.str_replace("_", " ", $row["contphotodesc"]).'</a><br />';
				} else { // ($ext == "gif") || ($ext == "jpg") || ($ext == "jpeg") || ($ext == "png") || //
        	if ($loop_tinyMCE_photos !== "")
          if (isset($send) && ($send == 'edit') && ($y != 'media'))
          $loop_tinyMCE_photos .= '</td><td align="center" style="border:none;">'; // '</div><div style="float:right;text-align:center;">'; //
          else {
            if (($i==ceil($contPhotocount/3)) || ($i==(2*(ceil($contPhotocount/3)))))
            $loop_tinyMCE_photos .= '</td></tr><tr><td align="center" style="border:none;">';
            else
            $loop_tinyMCE_photos .= '</td><td align="center" style="border:none;">';
          } 
					$loop_tinyMCE_photos .= '<a href="'.$mainurl.$row["contphotoimg"].'" target="_blank" title="'.$row["contphotodesc"].'" alt="'.$row["contphotodesc"].'"><img src="'.$mainurl.$row["contphotoimg"].'" width="50" height="50" hspace="5" vspace="5" border="0" alt="'.$row["contphotodesc"].'" /></a><br /><a href="'.$local_url.'&amp;send=delphoto&amp;contphotoId='.(isset($row["contphotorid"])?$row["contphotorid"]:$row["contphotoid"]).'" onclick="return confirm(\''.$confirmationeffacementString.'\');"><img src="'.$mainurl.'images/delete.gif" width="10" height="10" title="'.$effacerString.'" alt="'.$effacerString.'" border="0" /></a>';
				}
			}
			if ($loop_tinyMCE_photos != '')
...
// end photo list
// ########################## DOC ###############################
		$contDoccount = sql_nrows($tblcontdoc," ".(in_array("contdoclang",$doc_fields)?" WHERE contdoclang='$lg' ":'')); // WHERE contdoccontid='$x' 
		if ($tinyMCE === false) {
			$docListe = '<hr /><a href="'.$local_url.'&amp;send=uplddoc">'.$ajouterString.' 1 '.$docString.'</a>';
	// List docs from other pages only
		//	$contDoccount = sql_nrows($tblcontdoc,"");
			if ($contDoccount > '0') {
				...
				$read = @mysql_query("SELECT * FROM $tblcontdoc ".(in_array("contdoclang",$doc_fields)?" WHERE contdoclang='$lg' ":''));
				for ($i=0;$i<$contDoccount;$i++) {
					$row = @mysql_fetch_array($read);
  				$ext = explode('.',strrev($row["contdoc"]),2);
  				$ext = strtolower(strrev($ext[0]));
					$docListe .= '<a href="'.$mainurl.$row["contdoc"].'" target="_blank"><img src="'.$mainurl.'images/'.$ext.'logo.gif" width="16" height="16" border="0" alt="'.$ouvrirString.'" /></a> | <a href="#" onclick="insertDoc(\''.$row["contdoc"].'\');return(false)">'.str_replace("_", " ", $row["contdocdesc"]).'</a> | <a href="'.$local_url.'&amp;send=deldoc&amp;contphotoId='.(isset($row["contdocrid"])?$row["contdocrid"]:$row["contdocid"]).'" onclick="return confirm(\''.$confirmationeffacementString.'\');"><img src="'.$mainurl.'images/delete.gif" width="10" height="10" title="'.$effacerString.'" alt="'.$effacerString.'" border="0" /></a><br />';
				}
			} else {
			...
			$docListe = '<div style="float:left;width:49%;text-align:left;">| <a href="'.$local_url.'&amp;send=uplddoc">'.$ajouterString.' 1 '.$docString.'</a></div></div>'; // write javascript for import in tinymce.
      if (isset($send) && ($send == 'edit') && ($y != 'media'))
			$tinyMCE_docs = '<div style="float:right;overflow:auto;height:110px;min-width:300px;width:49%;text-align:left;">';
			else
			$tinyMCE_docs = '<div style="float:right;overflow:auto;height:300px;width:49%;min-width:300px;text-align:left;">';
			$read = @mysql_query("SELECT * FROM $tblcontdoc ".(in_array("contdoclang",$doc_fields)?" WHERE contdoclang='$lg' ":''));
			for ($i=0;$i<$contDoccount;$i++) {
				$row = @mysql_fetch_array($read);
				$ext = explode('.',strrev($row["contdoc"]),2);
				...
				$tinyMCE_docs .= '<a href="'.$local_url.'&amp;send=deldoc&amp;contdocId='.(isset($row["contdocrid"])?$row["contdocrid"]:$row["contdocid"]).'" onclick="return confirm(\''.$confirmationeffacementString.'\');"><img src="'.$mainurl.'images/delete.gif" width="10" height="10" title="'.$effacerString.'" alt="'.$effacerString.'" border="0" /></a> | <a href="'.$mainurl.$row["contdoc"].'" target="_blank"><img src="'.$mainurl.'images/'.$ext.'logo.gif" width="16" height="16" border="0" title="'.$ouvrirString.' '.$ext.'" alt="'.$ouvrirString.' '.$ext.'" /> '.str_replace("_", " ", $row["contdocdesc"]).'</a>';
			}
			$tinyMCE_docs .= '</div>';
...
#######################################################SEND PIC
		...
		for ($i=0;$i<$nof;$i++) {
			...
			if ($contphotoDesc != '') {
  			...
  			$ext = strtolower(strrev($ext[0]));
  			if	(in_array($ext,array_merge($array_img_ext,$array_swf_ext)))	$valid_ext = "true"	;
  			if	(!isset($valid_ext))	$valid_ext = "false"	;
  			$check = sql_nrows($tblcontphoto, " WHERE contphotoimg='".space2underscore($contphotoDesc).'.'.$ext." ");
  			if (!$contphotoDesc || !preg_match("/^[a-zA-Z0-9_-]+\$/",space2underscore($contphotoDesc)) || ($contphotoDesc == '') || 
    ...
    $error_img .= '</ul><a href="javascript:history.back()//">'.$retourString.'</a><br />';
          ...
          if (in_array($ext,$array_img_ext)) {
          	...
          	$userfile_name = space2underscore($contphotoDesc).'.'.$ext;//${$this_is."Desc"}.'.'.$ext;
    ...
    $error_img .= $error_request.'<br /><a href="'.$local_url.($x=='z'?'':'&amp;send=edit').'">'.$retourString.'</a>';
                ...
                if ($keylg == $default_lg) {
                  ...
                  $values .= ($values==''?'':',')."(NULL, 'Y', $dbtime, '$keylg', '".$insertread[0]."', '$admin_name', '$x', '$contphotoDesc', '$contphotoimg')";
                }
    $notice_img .= " $keylg ";
    					}
    					if ($values!='') {
    ...
    $notice_img .= $enregistrementString.' '.$effectueString.'<br />';
        				if	(in_array($ext,$array_img_ext))
    $notice_img .= '<img '.show_img_attr($insert_read_3).' src="'.$mainurl.$insert_read_3.'" align="center" vspace="5" hspace="5" title="'.$insertread[4].'" alt="'.$insertread[4].'" border="0" /><br />';
                else
    $notice_img .= '<a href="'.$mainurl.$insert_read_3.'" target="_blank" title="'.$insertread[4].'" alt="'.$insertread[4].'"><img src="'.$mainurl.'images/'.$ext.'logo.gif" width="16" height="16" vspace="5" hspace="5" title="'.$insertread[4].'" alt="'.$insertread[4].'" border="0" /> '.$insertread[4].'</a><br />';
              /*
  						if	(isset($logo)) {
  							$update = sql_update($dbtable,"SET contlogo='$insertread[0]' "," WHERE contpg='$x' AND contlang='$lg' ","contlogo");
  							if (($update[0] == '') || ($update[0] == '.')) {
    $error_img .= $error_request.'<a href="'.$local_url.($x=='z'?'':'&amp;send=edit').'">'.$retourString.'</a><br />';
  							}
  						} else {
    $notice_img .= ' | <a href="'.$local_url.'&amp;send=upldphoto">'.$ajouterString.' 1 '.$photoString.'</a><br /> <br />';
      $notice_img .= '<div style="border:1px solid green;padding:10px;text-align:left;"><h1>'.$rapportString.'</h1><a href="'.$mainurl.(stristr($insert_read_3,$filedir)?'':$filedir).substr($insert_read_3,0,-4).'.'.$ext.'" target="_blank">'.substr($insert_read_3,0,-4).'.'.$ext.'</a>';
        				if	(in_array($ext,$array_img_ext))
      $notice_img .= '<br /><a href="'.$mainurl.substr($insert_read_3,0,-4).'_big.'.$ext.'" target="_blank">'.substr($insert_read_3,0,-4).'_big.'.$ext.'</a><br /><a href="'.$mainurl.substr($insert_read_3,0,-4).'_ori.'.$ext.'" target="_blank">'.substr($insert_read_3,0,-4).'_ori.'.$ext.'</a>';
      $notice_img .= '<br /></div><br /><a href="'.$local_url.($x=='z'?'':'&amp;send=edit').'">'.$retourString.'</a></p>';
              }
              */
  					}
  				}
...
#######################################################DELETE PIC
		...
		if	( (!isset($contphotoId)) || !preg_match("/^[0-9]+\$/", $contphotoId) || ($contphotoId == '') )
      {Header("Location: $redirect");Die();}
		$contphotoId = strip_tags($contphotoId);
		$editcontphoto = sql_get($tblcontphoto, " WHERE contphotorid='$contphotoId' AND contphotolang='$lg' ", "contphotorid, contphotocontid, contphotoimg, contphotodesc");
		if ($editcontphoto[0] == '.')
      {Header("Location: $redirect");Die();}
		$edit_contphoto_2 = $editcontphoto[2];
		$deletequery = sql_del($tblcontphoto, " WHERE contphotorid='$contphotoId' ");
		if ($deletequery > '0') {
  $error .= '<p style="text-align: center">'.$error_request." WHERE contphotorid='$contphotoId' ".'<br /> <br /><a href="'.$local_url.($x=='z'?'':'&amp;send=edit').'">'.$retourString.'</a></p>';
		} else {
...
// || ($ext == 'swf') || ($ext == 'flv') || ($ext == 'wmv') || ($ext == 'avi') || ($ext == 'mov') || ($ext == 'rm') || ($ext == 'mpeg') || ($ext == 'mpg') || ($ext == 'asf') //
				$delurl = "../".(stristr($editcontphoto[2],$filedir)?$filedir:'').$editcontphoto[2];
				@unlink($getcwd.$delurl);
			}
      $notice .= $fichierString.' '.$effaceString.'<hr /><br />';
      if (!isset($notice)) {
        /*
  			if (isset($logo) || (sql_getone($dbtable,"WHERE contpg='$x' AND contlang='$lg' ","contlogo") == $contphotoId)) {
  				$update = sql_update($dbtable,"SET contlogo='' "," WHERE contpg='$x' AND contlang='$lg' ","contlogo");
  				if (($update[0] == '') || ($update[0] == '.'))
          $error .= '<p style="text-align: center">'.$error_request."SET contlogo=''  WHERE contpg='$x' AND contlang='$lg' ".'<br /> <br /></p>';
  			}
  			*/
        $notice .= '<br /></div><a href="'.$local_url.($x=='z'?'':'&amp;send=edit').'">'.$retourString.'</a><!-- <br /> <br />S`assurer que le lien ainsi que l`image sont retir&eacute;s du texte. --></p>';
        $notice .= '<div style="border:1px solid red;padding:10px;text-align:left;"><h1>'.$rapportString.'</h1>';
      }
      $notice .= substr($edit_contphoto_2,0,-(strlen($ext)+1)).'.'.$ext.'<br />';
  		if	(in_array($ext,$array_img_ext))
      $notice .= substr($edit_contphoto_2,0,-(strlen($ext)+1)).'_big.'.$ext.'<br />'.substr($edit_contphoto_2,0,-(strlen($ext)+1)).'_ori.'.$ext.'<br />';
		}
...
#######################################################SEND PIC
		...
		for ($i=0;$i<$nof;$i++) {
			...
			if ($contdocDesc != '') {
  			...
  			$get_ext = explode('.',strrev($userfile_name),2);
  			$ext = strtolower(strrev($get_ext[0]));
  			if ($contdocDesc == "")
  			$contdocDesc = space2underscore(strrev($get_ext[1]));
  			if	(in_array($ext,$array_doc_ext))	$valid_ext = "true"	;
  			if	(!isset($valid_ext))	$valid_ext = "false"	;
  			$check = sql_nrows($dbtable, " WHERE contdoc='".space2underscore($contdocDesc).'.'.$ext."' ");
    ...
    $error_img .= $error_request.'<br /><a href="'.$local_url.($x=='z'?'':'&amp;send=edit').'">'.$retourString.'</a><br />';
              ...
              if ($keylg == $default_lg) {
                ...
                $values .= ($values==''?'':',')."(NULL, 'Y', $dbtime, '$keylg', '".$insertread[0]."', '$admin_name', '$x', '$contdocDesc', '$contdoc')";
              }
    $notice_img .= " $keylg ";
    				}
    				if ($values!='') {
...
#######################################################DELETE PIC
	...
	/*
		if	((!isset($contdocId)) || !preg_match("/^[0-9]+\$/", $contdocId) || ($contdocId == ''))
		  {Header("Location: $redirect");Die();}
		*/
		$contdocId = strip_tags($contdocId);
		$editcontdoc = sql_get($tblcontdoc, " WHERE contdocrid='$contdocId' ", "contdocrid, contdoccontid, contdoc, contdocdesc");
		$edit_contdoc_2 = $editcontdoc[2];
		$deletequery = sql_del($tblcontdoc, " WHERE contdocrid='$contdocId' ");
		if ($deletequery > '0') {
  $error .= $error_delete.'<br /><a href="'.$local_url.($x=='z'?'':'&amp;send=edit').'">'.$retourString.'</a><br />';
		} else {
			$delurl = "../$edit_contdoc_2";
			@unlink($getcwd.$delurl);	
      $notice .= $fichierString.' '.$effaceString.'<br />';
      if (!isset($notice))
      $content .= '<a href="'.$local_url.'&amp;send=uplddoc">'.$ajouterString.' 1 '.$docString.'</a><br /> <br /><div style="border:1px solid red;padding:10px;text-align:left;"><h1>'.$rapportString.'</h1>'.$edit_contdoc_2.'</div><br /><a href="'.$local_url.($x=='z'?'':'&amp;send=edit').'">'.$retourString.'</a><br />';
		}
    if ($error == '') {// && stristr($_SERVER['REQUEST_URI'],$urladmin)) {
      $_SESSION['mv_notice'] = $notice;
      Header("Cache-Control: no-cache");
