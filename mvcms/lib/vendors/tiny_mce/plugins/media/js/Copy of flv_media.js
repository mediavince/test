...
function init() {
	...
	if (/mceItem(Flash|FlashVideo|ShockWave|WindowsMedia|QuickTime|RealMedia)/.test(ed.dom.getAttrib(fe, 'class'))) {
		pl = "x={" + fe.title + "};";
...
function insertMedia() {
	...
	fe = ed.selection.getNode();
	if (fe != null && /mceItem(Flash|FlashVideo|ShockWave|WindowsMedia|QuickTime|RealMedia)/.test(ed.dom.getAttrib(fe, 'class'))) {
...
function generatePreview(c) {
	...
	for (n in pl) {
		...
		if (n == 'src' && pl[n].indexOf('://') != -1) {
			if (f.media_type.options[f.media_type.selectedIndex].value == "flv") {
				h += '<param name="url" value="flv_player.swf?flvToPLay=' + pl[n] + '" />';
			} else {
	...
	for (n in pl) {
		if (n == 'src' && pl[n].indexOf('://') != -1) {
			if (f.media_type.options[f.media_type.selectedIndex].value == "flv") {
				h += n + '="flv_player.swf?flvToPLay=' + pl[n] + '" ';
			} else {
				h += n + '="' + pl[n] + '" ';
			}
		}
