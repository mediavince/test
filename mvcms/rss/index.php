...
if (!in_array(sql_getone($tblcont, "WHERE conttype='$k' AND contstatut='Y' ", "contpg"), array("", ".",)))
...
$sql = "SELECT DISTINCT htaccessdate,htaccessurl,htaccesstitle,htaccessitem,htaccesstype
	...
	." htaccessstatut='Y' AND htaccesslang='$lg' AND htaccesstype IN ('"
		...
		.(isset($rss_cont)&&$rss_cont===true
			?"UNION SELECT contupdate,conturl,conttitle,contpg,conttype FROM _cont WHERE "
				.(isset($rss_days)&&($rss_days>0)
...
$get_rss = mysqli_query($connection, $sql);
...
while($get_row = mysqli_fetch_array($get_rss)) {
	...
	$guid = ($get_row[4]!=''?$mainurl."?".$get_row[4]."Id=".$get_row[3]:$link);
  	echo '
<item>
<guid>'.$guid.'</guid>
